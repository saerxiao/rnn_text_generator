...
--local net = MultiLayerRnn(loader:vocabSize(), opt.nhidden, loader:vocabSize(), opt.nlayers, opt.model)
...
--  grads:zero()
--  
----  local input, target = loader:nextBatch('train') -- input: T x B x V, target: T x B, target[t][b]=number in [1,V]
--  local input, target = trainIter.nextBatch()  -- input: T x B x V, target: T x B, target[t][b]=number in [1,V]
--  if opt.gpuid > -1 then
...
--  
--  local output = net:forward(input) -- T x B x V
--  local loss = 0
----  local dloss = torch.Tensor(loader.T, B, loader:vocabSize()) -- T x B x V
--  local dloss = output.new()
--  dloss:resize(loader.T, B, loader:vocabSize())
--  for t = 1, loader.T do
...
--  local input, target = loader:nextBatch('train') -- input: B x T x V, target:  B x T, target[b][t]=number in [1,V]
  local input, target = trainIter.nextBatch()  -- input: B x T x V, target: B x T, target[b][t]=number in [1,V]
  if opt.gpuid > -1 then
    input, target = ship2gpu(input, target)
  end
...
--  local iter_per_epoch = math.ceil((loader.dataset.train:size(1) - 1) / loader.T / loader.batch_size) -- loader2
...
--    if opt.gpuid < 0 then
--      predict = predict:squeeze() -- T x B
--    end
--    for t = 1, target:size(1) do
--      for b = 1, target:size(2) do
--        if opt.gpuid < 0 then
--          if predict:dim() == 1 then
--            if predict[b] == target[t][b] then
--              hit = hit + 1
--            end
--          else
--            if predict[t][b] == target[t][b] then
--              hit = hit + 1
--            end
--          end          
--        else
--          if predict[t][b][1] == target[t][b] then
--            hit = hit + 1
--          end
--        end
--        
...
--local iter_per_epoch = math.ceil((loader.dataset.train:size(1) - loader.T)/loader.batch_size) -- for loader
--local iter_per_epoch = math.ceil((loader.dataset.train:size(1) - 1) / loader.T / loader.batch_size) -- for loader2
local iter_per_epoch = loader.nbatch.train -- for loader3
